...
class PPOPolicy(BasePolicy):
    ...
    def _build(self, lr_schedule: Callable) -> None:
        ...
        # TOM ADDED 
        # ---------------------
        self.transformer = Transformer(d_model=2764, 
                                       output_dim=2764, 
    ...
    def _get_latent(self, obs: th.Tensor) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        # [source_seq_len, batch_size, features]
        # Raw Obs shape: torch.Size([1, 30])
        # Features: torch.Size([1, 30])
        batch_size, dim = features.shape
        temp_obs = features.view(4, batch_size, -1)
        print(f"Shape of temp obs: {temp_obs.shape}")
        trans_output = self.transformer(temp_obs)
        # print(f"Shape of Transformer output: {trans_output.shape}")
        features = trans_output.view(batch_size, 276480)

        # print(f"Features: {features.shape}")
        latent_pi, latent_vf = self.mlp_extractor(features)
        # print(f"Post mlp extractor Pi: {latent_pi.shape}")
        # print(f"Post mlp extractor Vf: {latent_vf.shape}")
