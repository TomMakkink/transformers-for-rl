...
class PPO(BaseRLModel):
    ...
    def train(self, n_epochs: int, batch_size: int = 64) -> None:
        ...
        self.writer.add_scalar('Actor Loss',  np.mean(pg_losses), self.num_timesteps)
        self.writer.add_scalar('Critic Loss',  np.mean(value_losses), self.num_timesteps)
        self.writer.add_scalar('Kl', np.mean(approx_kl_divs), self.num_timesteps)
        self.writer.add_scalar('Entropy', np.mean(entropy_losses), self.num_timesteps)
        ...
        while self.num_timesteps < total_timesteps:
            ...
            if self.verbose >= 1 and log_interval is not None and iteration % log_interval == 0:
                ...
                if len(self.ep_info_buffer) > 0 and len(self.ep_info_buffer[0]) > 0:
                    ...
                    # TOM added
                    ep_rew_mean = self.safe_mean([ep_info['r'] for ep_info in self.ep_info_buffer])
                    ep_len_mean = self.safe_mean([ep_info['l'] for ep_info in self.ep_info_buffer])
                    self.writer.add_scalar('Mean Episode Reward', ep_rew_mean, self.num_timesteps)
                    self.writer.add_scalar('Mean Episode Length', ep_len_mean, self.num_timesteps)
